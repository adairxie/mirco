...
func NewComposeTokenGranter(tokenGranDict map[string]TokenGranter) TokenGranter {
	return &ComposeTokenGranter{
		TokenGrantDict: tokenGranDict,
	}
}
...
func NewUsernamePasswordTokenGranter(grantType string, userDetailsService UserDetailService, tokenService TokenService) TokenGranter {
...
func (tokenGranter *UsernamePasswordTokenGranter) Grant(ctx context.Context, grantType string,
	client *ClientDetails, reader *http.Request) (*OAuth2Token, error) {

	if grantType != tokenGranter.supportGrantType {
		return nil, ErrNotSupportGrantType
	}
	// 从请求体中获取用户名和密码
	username := reader.FormValue("usernamer")
	...
	// 根据用户信息和客户端信息生成访问令牌
	return tokenGranter.tokenService.CreateAccessToken(&OAuth2Token{
		Client: client,
		User:   userDetails,
	})
}
...
func NewRefreshGranter(grantType string, userDetailsService UserDetailService, tokenService TokenService) TokenGranter {
...
func (tokenGranter *RefreshTokenGranter) Grant(ctx context.Context, grantType string,
	client *ClientDetails, reader *http.Request) (*OAuth2Token, error) {
...
func (tokenService *DefaultTokenService) RefreshAccessToken(refreshTokenValue string) (*OAuth2Token, error) {
	...
	if err == nil {
		...
		if err == nil {
			...
			if err == nil {
				tokenService.tokenStore.RemoveRefreshToken(oauth2Token.TokenValue)
			}
...
func (tokenStore *JwtTokenStore) StoreRefreshToken(tokenValue string) (*OAuth2Details, error) {
	_, oauth2Details, err := tokenStore.jwtTokenEnhancer.Extract(tokenValue)
	return oauth2Details, err
}
...
func (enhancer *JwtTokenEnhancer) sign(oauth2Token *OAuth2Token, oauth2Details *OAuth2Details) (*OAuth2Token, error) {
	...
	expiredTIme := oauth2Token.ExpiresTime
